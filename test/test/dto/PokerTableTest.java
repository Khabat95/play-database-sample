...
public class PokerTableTest extends AbstractTest {
	...
	public void dtoConversion() {
		...
		PokerTable pokerTable = new PokerTable("Sydney", TableType.HOLDEM,
				TableLimit.NO_LIMIT, 10, new ArrayList<User>());
		assertEquals(pokerTable,
				PokerTable.fromDbPokerTableWithUsers(dbPokerTable));
		assertEquals(dbPokerTable, pokerTable.toDbPokerTable());
		assertEquals(pokerTable,
				PokerTable.fromDbPokerTableWithUsers(pokerTable
						.toDbPokerTable()));
		assertEquals(dbPokerTable,
				PokerTable.fromDbPokerTableWithUsers(dbPokerTable)
						.toDbPokerTable());
		// Different name
		pokerTable = new PokerTable("Melbourne", TableType.HOLDEM,
				TableLimit.NO_LIMIT, 10, new ArrayList<User>());
		assertNotEquals(pokerTable,
				PokerTable.fromDbPokerTableWithUsers(dbPokerTable));
		assertNotEquals(dbPokerTable, pokerTable.toDbPokerTable());
		// Different type
		pokerTable = new PokerTable("Sydney", TableType.OMAHA,
				TableLimit.NO_LIMIT, 10, new ArrayList<User>());
		assertNotEquals(pokerTable,
				PokerTable.fromDbPokerTableWithUsers(dbPokerTable));
		assertNotEquals(dbPokerTable, pokerTable.toDbPokerTable());
		...
		pokerTable = new PokerTable("Sydney", TableType.HOLDEM,
				TableLimit.POT_LIMIT, 10, new ArrayList<User>());
		assertNotEquals(pokerTable,
				PokerTable.fromDbPokerTableWithUsers(dbPokerTable));
		assertNotEquals(dbPokerTable, pokerTable.toDbPokerTable());
		// Different seatNumber
		pokerTable = new PokerTable("Sydney", TableType.HOLDEM,
				TableLimit.NO_LIMIT, 6, new ArrayList<User>());
		assertNotEquals(pokerTable,
				PokerTable.fromDbPokerTableWithUsers(dbPokerTable));
		assertNotEquals(dbPokerTable, pokerTable.toDbPokerTable());
		...
		List<PokerTable> list = PokerTable
				.fromDbPokerTableListWithUsers(dbList);
		assertEquals(dbList.size(), list.size());
		for (int i = 0; i < list.size(); i++) {
			assertEquals(dbList.get(i), list.get(i).toDbPokerTable());
			assertEquals(list.get(i),
					PokerTable.fromDbPokerTableWithUsers(dbList.get(i)));
		}
		...
		for (int i = 0; i < list.size(); i++) {
			assertEquals(dbList.get(i), list.get(i).toDbPokerTable());
			assertEquals(list.get(i),
					PokerTable.fromDbPokerTableWithoutUsers(dbList.get(i)));
		}
